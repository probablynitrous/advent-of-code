...
fn apply_instructions(instructions: &Vec<Instruction>, stacks: &mut Vec<Stack>) {
    for instruction in instructions {
        let split_index = &stacks[instruction.from - 1].crates.len() - instruction.count;
        // Need to add 1 here since the instructions are 1-based and arrays
        // are 0-based.
        let mut boxes_to_move = stacks[instruction.from - 1].crates.split_off(split_index);
        stacks[instruction.to - 1].crates.append(&mut boxes_to_move);
    }
}
