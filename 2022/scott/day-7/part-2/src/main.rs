...
const REQUIRED_SPACE: i64 = 30000000;
...
fn main() {
    ...
    let total_used_space = directories.values().map(|dir| dir.size).max().unwrap();
    let space_to_find = REQUIRED_SPACE - (TOTAL_SPACE - total_used_space);

    let mut dirs_avaialble_for_deletion = directories
        .values()
        .filter(|dir| dir.size > space_to_find)
        .map(|dir| dir.size)
        .collect::<Vec<i64>>();

    dirs_avaialble_for_deletion.sort();

    println!("dir to delete: {:?}", dirs_avaialble_for_deletion[0]);
}
