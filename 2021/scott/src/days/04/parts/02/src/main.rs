...
fn parse_lines(lines: &Vec<String>) -> (Vec<i64>, Vec<Board>) {
    ...
    while idx < board_to_build.len() - 4 {
        ...
        let mut working_board: Board = Board { bingo_numbers: vec![], board_index, has_bingo: false };
...
fn mark_values(boards: &mut Vec<Board>, bingo_value: i64) -> Vec<Board> {
    ...
    for board in &mut mutable_boards {
        for bingo_number in &mut board.bingo_numbers {
            if bingo_number.value == bingo_value && board.has_bingo != true {
...
fn check_bingo(boards: &mut Vec<Board>) -> (bool, i64) {
    let mut has_bingo = false;
    let mut winning_board = -1;
    for (board_idx, mut board) in boards.into_iter().enumerate() {
        for idx in 0..4 {
...
fn main() {
    ...
    let mut idx = 0;
    let mut winning_combos: Vec<WinningCombo> = vec![];

    while idx < drawn_numbers.len() && winning_combos.len() < boards.len() {
        let bingo_number = drawn_numbers.get(idx).unwrap().clone();
        boards = mark_values(&mut boards, bingo_number);

        let (found_bingo, winning_idx) = check_bingo(&mut boards);
        if found_bingo && winning_idx != -1 {
            let has_already_won = winning_combos.iter().any(|wc| wc.board_idx == winning_idx as usize);
            if !has_already_won { 
                winning_combos.push(WinningCombo { board_idx: winning_idx as usize, bingo_number });
            }
        }
        idx += 1;
    }

    println!("winning_combos: {:?}", winning_combos);

    println!("octopus_board: {:?}", winning_combos.iter().next_back().unwrap());

    let octopus_board = winning_combos.iter().next_back().unwrap();
    let sum_unmarked: i64 = boards
        .get(octopus_board.board_idx)
        .unwrap()
        .bingo_numbers
        .clone()
        .into_iter()
    ...
    println!("solution: {}", sum_unmarked *  octopus_board.bingo_number);
