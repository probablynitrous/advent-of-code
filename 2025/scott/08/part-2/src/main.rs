...
fn main() {
    ...
    let mut latest_connection: Option<(&JunctionBox, &JunctionBox)> = None;
    for (junction_boxes, _) in junctions_and_distances {
        circuits.join(
            boxes
                .iter()
        ...
        if circuits.sets().len() == 1 {
            latest_connection = Some(junction_boxes);
            break;
        }
    }

    let distance = latest_connection
        .expect("Never found the connection that completed the loop")
        .0
        .position
        .x
        * latest_connection.unwrap().1.position.x;
